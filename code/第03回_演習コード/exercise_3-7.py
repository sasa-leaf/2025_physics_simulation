...
def update():
  ...
  for i in range(N_particles):
    ...
    # 壁面との衝突力
    # 下の壁面との衝突
    if particles_pos[i].y - 0.5*psize < domain[0].y and particles_vel[i].y < 0:
      penetration = domain[0].y - (particles_pos[i].y - 0.5*psize)
      particles_force[i].y += (penetration*10000.0 - (1.0+1.0)*particles_mass[i]/dt[None]*particles_vel[i].y) - particles_mass[i]*gravity.y
    # 上の壁面との衝突
    if particles_pos[i].y + 0.5*psize > domain[1].y and particles_vel[i].y > 0:
      penetration = (particles_pos[i].y + 0.5*psize) - domain[1].y
      particles_force[i].y += (-penetration*10000.0 - (1.0+1.0)*particles_mass[i]/dt[None]*particles_vel[i].y) - particles_mass[i]*gravity.y
    # 左の壁面との衝突
    if particles_pos[i].x - 0.5*psize < domain[0].x and particles_vel[i].x < 0:
      penetration = domain[0].x - (particles_pos[i].x - 0.5*psize)
      particles_force[i].x += (penetration*10000.0 - (1.0+1.0)*particles_mass[i]/dt[None]*particles_vel[i].x) - particles_mass[i]*gravity.y
    # 右の壁面との衝突
    if particles_pos[i].x + 0.5*psize > domain[1].x and particles_vel[i].x > 0:
