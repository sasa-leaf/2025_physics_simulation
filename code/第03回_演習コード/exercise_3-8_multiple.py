...
rigids_fixcenter = ti.field(ti.i32,shape=(N_rigids))
...
def update():
  ...
  for i in range(N_particles):
    ...
    if mouse_flag[None]:
      ...
      dist = r.norm()
      overlap = mouse_radius - dist
      if overlap > 0:
        force_magnitude = 100.0*overlap
        particles_force[i] += force_magnitude * r.normalized()
    # 壁面との衝突力
    # 下の壁
    overlap = domain[0].y - (particles_pos[i].y - 0.5 * psize)
    if overlap > 0:
        particles_force[i].y += 100.0 * overlap
    # 上の壁
    overlap = (particles_pos[i].y + 0.5 * psize) - domain[1].y
    if overlap > 0:
        particles_force[i].y -= 100.0 * overlap
    # 左の壁
    overlap = domain[0].x - (particles_pos[i].x - 0.5 * psize)
    if overlap > 0:
        particles_force[i].x += 100.0 * overlap
    # 右の壁
    overlap = (particles_pos[i].x + 0.5 * psize) - domain[1].x
    if overlap > 0:
        particles_force[i].x -= 100.0 * overlap    
    ...
    for j in range(N_particles):
      if particles_rigid_id[i] == particles_rigid_id[j]:
        ...
        r_ij = particles_pos[i] - particles_pos[j]
        dist_ij = r_ij.norm()
        overlap_ij = psize - dist_ij
        if overlap_ij > 0:
          force_magnitude_ij = 100.0*overlap_ij
          particles_force[i] += force_magnitude_ij * r_ij.normalized()
