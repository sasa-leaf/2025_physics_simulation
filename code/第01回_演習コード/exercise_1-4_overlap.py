...
def update():
    ...
    for i in range(N_points):
        ...
        # x方向の壁衝突
        if p.x < (minx + radius):
            p.x = 2.0 * (minx + radius) - p.x 
            v.x = -v.x
        elif p.x > (maxx - radius):
            p.x = 2.0 * (maxx - radius) - p.x
            v.x = -v.x

        # y方向の壁衝突
        if p.y < (miny + radius):
            p.y = 2.0 * (miny + radius) - p.y
            v.y = -v.y
        elif p.y > (maxy - radius):
            p.y = 2.0 * (maxy - radius) - p.y
            v.y = -v.y
        ...
        for j in range(N_points):
            if i != j:
                ...
                if dist_ij < 2.0 * radius:
                    ...
                    # 重なり部分を互いに半分ずつ移動させる
                    overlap = 2.0 * radius - dist_ij
                    p -= n_ij * (overlap * 0.5)
                    points_pos[j] += n_ij * (overlap * 0.5)
