...
def update():
  ...
  for i in range(N_points):
    ...
    # 壁面の反発力に，抗力を追加
    # F_wall = -2m/dt*{n*v}n - m*(n*g)n
    # 下の壁面との衝突
    if abs(domain[0].y-points_pos[i].y) <= points_radius[i] and points_vel[i].y < 0:
      points_force[i].y += -(1.0+collision_coef)*points_mass[i]/dt*points_vel[i].y - points_mass[i]*gravity.y
    # 上の壁面との衝突
    ...
    if abs(domain[0].x-points_pos[i].x) <= points_radius[i] and points_vel[i].x < 0:
      points_force[i].x += -(1.0+collision_coef)*points_mass[i]/dt*points_vel[i].x - points_mass[i]*gravity.y
    # 右の壁面との衝突
  ...
  # 位置の更新
  for i in range(N_points):
    points_pos[i] += points_vel[i]*dt - 0.5*points_acc[i]*dt*dt
  # エネルギー
  energy[None] = 0.0
  for i in range(N_points):
