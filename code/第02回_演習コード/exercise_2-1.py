...
def update():
  ...
  for i in range(N_points):
    ...
    if abs(domain[0].y-points_pos[i].y) <= points_radius[i] and points_vel[i].y < 0:
      points_force[i].y += -2.0*points_mass[i]/dt*points_vel[i].y - points_mass[i]*gravity.y
    # 上の壁面との衝突
    ...
    if abs(domain[0].x-points_pos[i].x) <= points_radius[i] and points_vel[i].x < 0:
      points_force[i].x += -2.0*points_mass[i]/dt*points_vel[i].x - points_mass[i]*gravity.y
    # 右の壁面との衝突
