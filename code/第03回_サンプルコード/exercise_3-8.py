...
for k in range(N_rigids):
  rigids_pset[k] = create_rectangle(-0.25+0.5*k,0.0,0.4,0.4)
  rigids_mass[k] = 2.0+2.0*k
...
def update():
  ...
  for i in range(N_particles):
    ...
    if mouse_flag[None]:
      ...
      if dist_ij < mouse_radius+psize*0.5:
        ...
        if tmp1 < 0.0:
          particles_force[i] += normal_ij*(particles_mass[i]*10*tmp1/dt_max)
        tmp2 = mouse_radius+psize*0.5-dist_ij # 重なりの長さ
        if tmp2 > 0.0:
          particles_force[i] -= normal_ij*(particles_mass[i]*10*tmp2*0.1/dt_max**2)
    # 粒子同士の衝突力
    for j in range(N_particles):
      if particles_rigid_id[i] == particles_rigid_id[j]:
        continue
      pos_ij = particles_pos[j]-particles_pos[i]
      len_ij = psize-pos_ij.norm() # 重なりの長さ
      if len_ij > 0.0:
        normal_ij = pos_ij.normalized()
        m_i = particles_mass[i]
        m_j = particles_mass[j]
        m_ij = 2*m_i*m_j/(m_i+m_j)
        particles_force[i] -= normal_ij*(m_ij*len_ij/dt_max**2)
        tmp = normal_ij.dot(particles_vel[j]-particles_vel[i]) # 法線方向の相対速度
        if tmp < 0.0:
          particles_force[i] += normal_ij*(m_ij*10*tmp/dt_max)
        if True: # 摩擦力を考慮する場合
          tau_ij = ti.Vector([-normal_ij.y,normal_ij.x]) # 接線ベクトル
          tmp2 = tau_ij.dot(particles_vel[j]-particles_vel[i]) # 接線方向の相対速度
          particles_force[i] += tau_ij*(m_ij*tmp2/dt_max)
    # 壁面との衝突力
    for j in range(4):
      pos_i = particles_pos[i]
      # 壁面の位置と法線ベクトル
      pos_j = ti.Vector([0.0,0.0])
      normal_ij = ti.Vector([0.0,0.0])
      if j == 0: # 左
        pos_j = (domain[0].x,pos_i.y)
        normal_ij = (-1.0,0.0)
      elif j == 1: # 右
        pos_j = (domain[1].x,pos_i.y)
        normal_ij = (1.0,0.0)
      elif j == 2: # 下
        pos_j = (pos_i.x,domain[0].y)
        normal_ij = (0.0,-1.0)
      elif j == 3: # 上
        pos_j = (pos_i.x,domain[1].y)
        normal_ij = (0.0,1.0)
      len_ij = psize*0.5-normal_ij.dot(pos_j-pos_i) # めり込み深さ
      if len_ij > 0.0:
        particles_force[i] -= normal_ij*(particles_mass[i]*len_ij/dt_max**2)
        tmp = normal_ij.dot(-particles_vel[i]) # 法線方向の相対速度
        if tmp < 0.0:
          particles_force[i] += normal_ij*(particles_mass[i]*10*tmp/dt_max)
        if True: # 摩擦力を考慮する場合
          tau_ij = ti.Vector([-normal_ij.y,normal_ij.x]) # 接線ベクトル
          tmp2 = tau_ij.dot(-particles_vel[i]) # 接線方向の相対速度
          particles_force[i] += tau_ij*(particles_mass[i]*tmp2/dt_max)
  # 力の和と力のモーメントの和の計算
  ...
  for i in range(N_particles):
    ...
    rigids_moment[k] += (particles_pos[i]-rigids_rcenter[k]).cross(particles_force[i])
  # 重心速度と角速度の更新
  for k in range(N_rigids):
    if not rigids_fixcenter[k]:
  ...
  energy[None] = 0.0
  for i in range(N_particles):
    energy[None] += 0.5*particles_mass[i]*particles_vel[i].norm_sqr()
...
while gui.running:
  ...
  gui.text(f'Energy: {energy[None]:.6f}',(0.0,0.950),font_size=20,color=0xFFFFFF)
  gui.show()
