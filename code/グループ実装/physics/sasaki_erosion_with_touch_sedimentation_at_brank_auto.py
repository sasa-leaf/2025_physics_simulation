...
solidification_time_threshold = 0.4 # 停滞がこの秒数続いたら壁化する

# 空間成長（空隙埋め）モデル
growth_prob = 0.007           # 1ステップあたりに成長する確率
...
y_left = p_y_max - psize * 4.0          # 左上の高さ
...
top_inflow_vel = ti.Vector([-1.5, -0.5])
            ...
            for l in range(table_cnt[bx,by]):
                ...
                if particles_type[j] == type_wall:
                    if dist_ij < erosion_dist:
                        ...
                        threshold = stagnant_vel_threshold-0.8
                        if vel_normal > threshold:
                            # 確率で壁を流体に変化させる
                            if ti.random() < erosion_prob * vel_normal / threshold:
                                particles_type[j] = type_fluid
                                # 速度を少し与える
                                particles_vel[j] = particles_vel[i] * 0.1
