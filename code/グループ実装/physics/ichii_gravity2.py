...
def update():
    ...
    for i in range(N_particles):
        ...
        if particles_type[i] == type_fluid:# 以下Gemini
            ...
            # 共通項を計算
            cosine = ti.sqrt(1 / (1 + tangent**2))
            sine = ti.sqrt(tangent**2 / (1 + tangent**2)) * sgn(tangent)
            ...
            # 流れ方向（接線方向）
            particles_force[i].x += fluid_density * gravity.norm() * sine
            particles_force[i].y += fluid_density * gravity.norm() * -1.0 * cosine
            
            # 壁方向（法線方向）
            ratio = gravityratioabs(fluid_x, fluid_y)
            sgn_xcenter = sgn(fluid_x - amplitude * ti.sin(frequency * fluid_y * math.pi))
            
            particles_force[i].x += fluid_density * -sgn_xcenter * ratio * cosine
            particles_force[i].y += fluid_density * -sgn_xcenter * ratio * ti.abs(sine) # 以上Gemini
