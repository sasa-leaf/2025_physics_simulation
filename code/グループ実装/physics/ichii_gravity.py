...
# 重力
gravity = ti.Vector([0.0,-100.0]) # 重力加速度[m/s^2]
...
# 堆積モデル
stagnant_vel_threshold = 1.0    # この速度を下回ったら「停滞」とみなす
solidification_time_threshold = 10.0 # 停滞がこの秒数続いたら壁化する

# 空間成長（空隙埋め）モデル
growth_prob = 0.00001           # 1ステップあたりに成長する確率
...
# @ti.func
# def gravityratio(x, y):
#     # math.sin -> ti.sin, math.pi -> ti.pi に変更
#     res = 0
#     if ti.abs(x - amplitude * ti.sin(frequency * y * math.pi)) < width_bottom:
#         res = 1.0 * gravity.norm()
#     else:
#         res = 0.0
#     return res
...
def update():
    ...
    for i in range(N_particles):
        ...
        if particles_type[i] == type_wall or particles_type[i] == type_ghost or particles_type[i] == type_strong_wall:
            continue
        # # 重力
        # if particles_type[i] == type_fluid:# 以下Gemini
        #     fluid_x = particles_pos[i].x
        #     fluid_y = particles_pos[i].y
            
        #     # math.cos -> ti.cos
        #     tangent = amplitude * frequency * math.pi * ti.cos(frequency * fluid_y * math.pi) #dx/dyのつもり
        #     # x = amplitude * ti.sin(frequency * fluid_y * math.pi)

        #     # 共通項を計算
        #     inv_sqrt1 = ti.sqrt(1 / (1 + tangent**2))
        #     inv_sqrt2 = ti.sqrt(tangent**2 / (1 + tangent**2))
            
        #     # # gravity が Vector の場合は、スカラー成分（.yなど）を指定する
        #     # # ここでは gravity_val と仮定
        #     # g_val = gravity if isinstance(gravity, float) else gravity.y 

        #     # 流れ方向（接線方向）
        #     particles_force[i].x += fluid_density * gravity.norm() * sgn(tangent) * inv_sqrt1
        #     particles_force[i].y += fluid_density * gravity.norm() * -1.0 * inv_sqrt2
            
        #     # 壁方向（法線方向）
        #     ratio = gravityratio(fluid_x, fluid_y)
        #     diff_x = sgn(fluid_x - amplitude * ti.sin(frequency * fluid_y * math.pi))
            
        #     particles_force[i].x += fluid_density * ratio * diff_x * sgn(tangent) * inv_sqrt2
        #     particles_force[i].y += fluid_density * ratio * -1.0 * inv_sqrt1 # 以上Gemini
            ...
            for l in range(table_cnt[bx,by]):
                ...
                if particles_type[j] == type_wall:
                    if dist_ij < erosion_dist:
                        # 法線ベクトル（流体i -> 壁j の向き）を計算
                        normal_ij = pos_ij / dist_ij
                        # 速度ベクトルと法線ベクトルの内積をとる
                        vel_normal = particles_vel[i].dot(normal_ij)

                        # 壁に向かう速度成分が閾値を超えている場合のみ侵食
                        if vel_normal > stagnant_vel_threshold-0.8:
                            # 確率で壁を流体に変化させる
    ...
    for i in range(N_particles):
        if particles_type[i] == type_fluid:
            # 速度が閾値より小さい場合、時間をカウントアップ
            if particles_vel[i].norm() < stagnant_vel_threshold:
                particles_stagnant_time[i] += dt[None]
                
                # 指定時間を超えたら壁化する
                if particles_stagnant_time[i] > solidification_time_threshold:
                    particles_type[i] = type_wall
                    particles_vel[i] = (0.0, 0.0) # 壁になったので停止させる
                    particles_stagnant_time[i] = 0.0 # タイマーリセット
            else:
                # 動いている場合はタイマーをリセット
                particles_stagnant_time[i] = 0.0

    # 壁の近傍が空いていて、流体がいないなら壁化
    for i in range(N_particles):
        # 既存の壁粒子を起点にする
        if particles_type[i] == type_wall:
            # 確率で成長判定を行う（これにより「一定時間」の遅延を表現）
            if ti.random() < growth_prob:
                # 任意の隣接方向をターゲットにする
                angle = ti.random() * 2.0 * 3.1415926535
                growth_dir = ti.Vector([ti.math.cos(angle), ti.math.sin(angle)])
                pos_target = particles_pos[i] + growth_dir * psize 

                # ターゲットが計算領域内かチェック
                if pos_target.x > domain[0].x and pos_target.x < domain[1].x and \
                   pos_target.y > domain[0].y and pos_target.y < domain[1].y:
                    
                    can_grow = True
                    
                    # ターゲット位置周辺を検索
                    bx_t = int((pos_target - domain[0]).x / re)
                    by_t = int((pos_target - domain[0]).y / re)
                    
                    # 近傍探索
                    for bx, by in ti.ndrange((bx_t - 1, bx_t + 2), (by_t - 1, by_t + 2)):
                        if bx < 0 or bx >= Nx_buckets or by < 0 or by >= Ny_buckets:
                            continue
                        for l in range(table_cnt[bx, by]):
                            j = table_data[bx, by, l]
                            
                            dist_grow = (particles_pos[j] - pos_target).norm()
                            
                            # 条件A: すでに誰か（壁や粒子）がいるなら生成しない
                            if dist_grow < psize * 0.8:
                                can_grow = False
                            
                            # 条件B: 近くに「流体」がいるなら生成しない（流れがある場所は埋めない）
                            if particles_type[j] == type_fluid and dist_grow < growth_suppress_dist:
                                can_grow = False
                    
                    # 条件を満たせばゴースト粒子を壁として出現させる
                    if can_grow:
                        # 空きスロット（ゴースト）を探す
                        j_ghost = -1
                        # ※本来は並列処理内で全探索は重いが、発生頻度(growth_prob)が低いので許容
                        for k in range(N_particles):
                            # atomic_cas等は使わず簡易的に検索
                            if particles_type[k] == type_ghost:
                                # 競合を避けるため atomic_cmpxchg のような処理が望ましいが
                                # 簡易的に atomic_min で確保を試みる
                                old_type = ti.atomic_min(particles_type[k], type_wall)
                                if old_type == type_ghost:
                                    j_ghost = k
                                    break
                        
                        if j_ghost != -1:
                            particles_type[j_ghost] = type_wall
                            particles_pos[j_ghost] = pos_target
                            particles_vel[j_ghost] = (0.0, 0.0)
                            particles_color[j_ghost] = 0x808080 # 壁色
